...
class ResourceConfig(Base):
    ...
    instance_preferences: An ordered list of ML compute instance types for the training job, in priority order. SageMaker launches the training job on the first instance type in the list that has available capacity. If capacity is insufficient, SageMaker evaluates the next instance type in the preferred list. Exactly one instance type is selected for the job.  InstancePreferences is mutually exclusive with InstanceType, InstanceGroups, InstancePlacementConfig, and EnableManagedSpotTraining, and supports only Flexible Training Plans (FTP) and On-Demand capacity.
    selected_instance_type: The instance type that SageMaker selected for the job from the provided InstancePreferences. The job is billed for this instance type and count. Returned by  DescribeTrainingJob  after an instance type is selected. This field is read-only and isn't accepted in CreateTrainingJob requests.
    selected_instance_count: The number of instances of SelectedInstanceType that the training job launched with. The job is billed for this instance type and count. Returned by DescribeTrainingJob after an instance type is selected. This field is read-only and isn't accepted in CreateTrainingJob requests.
    """
...
class ProcessingClusterConfig(Base):
    ...
    instance_preferences: An ordered list of ML compute instance types for the processing job, in priority order. Amazon SageMaker launches the job on the first instance type in the list that has available capacity. If capacity is insufficient, Amazon SageMaker evaluates the next instance type in the list. Exactly one instance type is selected for the job.  InstancePreferences is mutually exclusive with InstanceType.
    selected_instance_type: The instance type that Amazon SageMaker selected for the job from InstancePreferences. Returned by  DescribeProcessingJob  after an instance type is selected. This field is read-only and isn't accepted in CreateProcessingJob requests.
    selected_instance_count: The number of instances of SelectedInstanceType that the job launched with. The job is billed for this instance type and count. Returned by DescribeProcessingJob after an instance type is selected. This field is read-only and isn't accepted in CreateProcessingJob requests.
    """
...
class HubContent(Base):
    ...
    hub_content_markdown: A string that provides a description of the hub content. This string can include links, tables, and standard markdown formatting.
    ...
    sage_maker_public_hub_content_arn: The Amazon Resource Name (ARN) of the public hub content.
...
class SearchRecord(Base):
    ...
    hub_content: The properties of a hub content resource.
    """
    ...
    hub_content: Optional[HubContent] = Unassigned()
