...
Tests the logic that ensures sagemaker>=3.2.0 is included in remote function dependencies
...
from sagemaker.core.remote_function.job import _ensure_sagemaker_dependency, _check_sagemaker_version_compatibility
...
class TestEnsureSagemakerDependency(unittest.TestCase):
    ...
    def test_no_dependencies_creates_temp_requirements_file(self):
        ...
        with open(result, "r") as f:
            content = f.read()
        self.assertIn("sagemaker>=3.2.0,<4.0.0", content)
    ...
    def test_no_dependencies_file_has_correct_format(self):
        ...
        self.assertEqual(lines[0].strip(), "sagemaker>=3.2.0,<4.0.0")
    ...
    def test_version_constraint_format(self):
        ...
        self.assertIn(">=3.2.0", content)
        self.assertIn("<4.0.0", content)
...
class TestCheckSagemakerVersionCompatibility(unittest.TestCase):
    ...
    # ===== BAD CASES (should raise ValueError) =====
    ...
    def test_multiple_version_specifiers_good_with_lower_bound(self):
        ...
        # Should not raise - <3.300.0 decrements to 3.299.0 which is >= 3.2.0
        _check_sagemaker_version_compatibility("sagemaker>=3.0.0,<3.300.0")

    def test_multiple_version_specifiers_bad(self):
        """Test multiple version specifiers that are bad."""
        # Should raise - <3.2.0 decrements to 3.1.0 which is < 3.2.0 (HMAC)
        with self.assertRaises(ValueError):
            _check_sagemaker_version_compatibility("sagemaker>=3.0.0,<3.2.0")

    def test_v3_good_greater_than_31(self):
        """Test V3 greater than 3.1.0 (not checked - treat as lower bound only)."""
