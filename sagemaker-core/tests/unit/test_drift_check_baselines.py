...
def test_drift_check_baselines_with_model_quality():
    ...
    baselines = DriftCheckBaselines(
        model_statistics=mock_statistics,
        model_constraints=mock_constraints
    )
...
def test_drift_check_baselines_with_model_data_quality():
    ...
    baselines = DriftCheckBaselines(
        ...
        model_data_constraints=mock_constraints
    )
    
    request_dict = baselines._to_request_dict()
    
    assert "ModelDataQuality" in request_dict
    assert request_dict["ModelDataQuality"]["Statistics"] == {"S3Uri": "s3://bucket/data-stats.json"}
    assert request_dict["ModelDataQuality"]["Constraints"] == {"S3Uri": "s3://bucket/data-constraints.json"}
...
def test_drift_check_baselines_with_explainability():
    ...
    mock_constraints = Mock()
    mock_constraints._to_request_dict.return_value = {"S3Uri": "s3://bucket/explain-constraints.json"}
    mock_config = Mock()
    mock_config._to_request_dict.return_value = {"S3Uri": "s3://bucket/explain-config.json"}
    
    baselines = DriftCheckBaselines(
        explainability_constraints=mock_constraints,
        explainability_config_file=mock_config
    )
    
    request_dict = baselines._to_request_dict()
    
    assert "Explainability" in request_dict
    assert request_dict["Explainability"]["Constraints"] == {"S3Uri": "s3://bucket/explain-constraints.json"}
    assert request_dict["Explainability"]["ConfigFile"] == {"S3Uri": "s3://bucket/explain-config.json"}
...
def test_drift_check_baselines_all_parameters():
    ...
    mock_model_constraints._to_request_dict.return_value = {"S3Uri": "s3://bucket/model-constraints.json"}
    ...
    mock_data_constraints._to_request_dict.return_value = {"S3Uri": "s3://bucket/data-constraints.json"}
    ...
    mock_explain_constraints._to_request_dict.return_value = {"S3Uri": "s3://bucket/explain-constraints.json"}
