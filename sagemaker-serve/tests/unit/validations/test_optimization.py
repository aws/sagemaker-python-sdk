    ...
    def test_gpu_smoothquant_without_compilation(self):
        with pytest.raises(ValueError, match="must be provided with Compilation"):
            _validate_optimization_configuration(
                is_jumpstart=False,
                instance_type="ml.g5.xlarge",
                quantization_config={"OverrideEnvironment": {"OPTION_QUANTIZE": "smoothquant"}},
                compilation_config=None,
                sharding_config=None,
                speculative_decoding_config=None,
            )
...
class TestConstants:
    ...
    def test_trt_configuration(self):
        ...
        assert (
            TRT_CONFIGURATION["optimization_combination"].optimization_container
            == _OptimizationContainer.TRT
        )
    ...
    def test_vllm_configuration(self):
        ...
        assert (
            VLLM_CONFIGURATION["optimization_combination"].optimization_container
            == _OptimizationContainer.VLLM
        )
    ...
    def test_neuron_configuration(self):
        ...
        assert (
            NEURON_CONFIGURATION["optimization_combination"].optimization_container
            == _OptimizationContainer.NEURON
        )
