...
class TestTensorflowServingPrepare(unittest.TestCase):
    ...
    @patch(
        "sagemaker.serve.model_server.tensorflow_serving.prepare._get_saved_model_path_for_tensorflow_and_keras_flavor"
    )
    ...
    @patch("shutil.copy2")
    def test_prepare_for_tf_serving_success(
        self, mock_copy, mock_capture, mock_hash, mock_get_saved, mock_move
    ):
    ...
    @patch("sagemaker.serve.model_server.tensorflow_serving.prepare.capture_dependencies")
    @patch("shutil.copy2")
    def test_prepare_for_tf_serving_no_saved_model(
        self, mock_copy, mock_capture, mock_hash, mock_get_saved
    ):
    ...
    @patch("sagemaker.serve.model_server.tensorflow_serving.prepare.capture_dependencies")
    @patch("shutil.copy2")
    def test_prepare_for_tf_serving_with_shared_libs(
        self, mock_copy, mock_capture, mock_hash, mock_get_saved, mock_move
    ):
    ...
    @patch("sagemaker.serve.model_server.tensorflow_serving.prepare.capture_dependencies")
    @patch("shutil.copy2")
    def test_prepare_for_tf_serving_invalid_dir(
        self, mock_copy, mock_capture, mock_hash, mock_get_saved
    ):
        ...
        with self.assertRaises(Exception) as context:
            prepare_for_tf_serving(model_path=str(file_path), shared_libs=[], dependencies={})
