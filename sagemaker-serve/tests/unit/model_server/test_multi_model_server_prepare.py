...
class TestMultiModelServerPrepare(unittest.TestCase):
    ...
    def test_prepare_mms_js_resources(self, mock_create_dir, mock_copy_js):
        ...
        mock_copy_js.assert_called_once_with(
            "s3://bucket/model.tar.gz", "test-js-id", mock_code_dir
        )
    ...
    @patch("shutil.copy2")
    def test_prepare_for_mms_creates_structure(
        self, mock_copy, mock_capture, mock_hash, mock_input
    ):
    ...
    @patch("sagemaker.serve.model_server.multi_model_server.prepare.capture_dependencies")
    @patch("shutil.copy2")
    def test_prepare_for_mms_raises_on_invalid_dir(
        self, mock_copy, mock_capture, mock_hash, mock_input
    ):
    ...
    @patch("sagemaker.serve.model_server.multi_model_server.prepare.capture_dependencies")
    @patch("shutil.copy2")
    def test_prepare_for_mms_copies_shared_libs(
        self, mock_copy, mock_capture, mock_hash, mock_input
    ):
