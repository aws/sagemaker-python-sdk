...
class TestArtifactPathPropagation(unittest.TestCase):
    ...
    def test_base_model_artifact_uri_propagated_to_inference_component(
        ...
        """Test that base model uses model_name in InferenceComponentSpecification for non-LORA deployment."""
        # Setup: Model customization deployment
        mock_is_model_customization.return_value = True
        mock_fetch_peft.return_value = "FULL"

        # Setup: Artifact URI resolution (called during build, not deploy)
        ...
        builder.built_model.model_name = "test-built-model-name"
        ...
        # Verify: InferenceComponent.create was called with model_name from built_model
        ...
        # Verify model_name is used in the specification (non-LORA path uses model_name)
        assert ic_spec.model_name == "test-built-model-name"
    ...
    def test_fine_tuned_model_artifact_uri_propagated_to_inference_component(
        ...
        """Test that fine-tuned model uses model_name in InferenceComponentSpecification for non-LORA deployment."""
        # Setup: Model customization deployment
        mock_is_model_customization.return_value = True
        mock_fetch_peft.return_value = "FULL"
        ...
        # Mark as built with cached compute requirements
        builder.built_model = Mock()
        builder.built_model.model_name = "test-fine-tuned-model-name"
        from sagemaker.core.shapes import InferenceComponentComputeResourceRequirements
        ...
        # Verify model_name is used in the specification (non-LORA path uses model_name)
        assert ic_spec.model_name == "test-fine-tuned-model-name"
    ...
    def test_lora_adapter_no_artifact_uri_propagated(
        ...
        # Mark as built with cached compute requirements
        builder.built_model = Mock()
        builder.built_model.model_name = "test-base-model-name"
        from sagemaker.core.shapes import InferenceComponentComputeResourceRequirements
    ...
    def test_environment_variables_propagated_with_artifact_path(
        ...
        """Test that non-LORA deployment uses model_name and compute requirements are set."""
        # Setup: Model customization deployment
        mock_is_model_customization.return_value = True
        mock_fetch_peft.return_value = "FULL"
        ...
        # Mark as built with cached compute requirements
        builder.built_model = Mock()
        builder.built_model.model_name = "test-model-with-env-vars"
        from sagemaker.core.shapes import InferenceComponentComputeResourceRequirements
        ...
        # Verify model_name is used in the specification (non-LORA path uses model_name)
        assert ic_spec.model_name == "test-model-with-env-vars"

        # Verify compute requirements are set
        assert ic_spec.compute_resource_requirements.min_memory_required_in_mb == 16384
        assert ic_spec.compute_resource_requirements.number_of_cpu_cores_required == 8.0
        assert ic_spec.compute_resource_requirements.number_of_accelerator_devices_required == 4.0
