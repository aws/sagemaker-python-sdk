...
class TestBuildForModelServer(unittest.TestCase):
    ...
    def test_build_for_model_server_without_model_raises(self):
        ...
        builder = ModelBuilder(
            role_arn=MOCK_ROLE_ARN, sagemaker_session=self.mock_session, image_uri=MOCK_IMAGE_URI
        )
        builder.model = None
...
class TestBuildForTorchServe(unittest.TestCase):
    ...
    def test_build_for_torchserve_with_model_object(
        self, mock_get_base, mock_detect_fw, mock_prepare, mock_create, mock_save_pkl
    ):
    ...
    def test_build_for_torchserve_with_hf_model_id(
        self, mock_is_js, mock_prepare, mock_create, mock_save_pkl
    ):
    ...
    @patch("sagemaker.serve.model_builder.ModelBuilder._is_jumpstart_model_id")
    def test_build_for_torchserve_with_hf_token(
        self, mock_is_js, mock_save, mock_prepare, mock_create
    ):
...
class TestBuildForJumpStart(unittest.TestCase):
    ...
    def test_build_for_jumpstart_unsupported_image_raises(self, mock_prepare, mock_get_kwargs):
        """Test that unsupported JumpStart image in LOCAL_CONTAINER mode raises error."""
        ...
        self.assertIn("Local container mode is not yet supported", str(context.exception))
        self.assertIn("Mode.SAGEMAKER_ENDPOINT", str(context.exception))

    @patch("sagemaker.core.jumpstart.factory.utils.get_init_kwargs")
    @patch("sagemaker.serve.model_builder.ModelBuilder._create_model")
    @patch("sagemaker.serve.model_builder.ModelBuilder._prepare_for_mode")
    def test_build_for_jumpstart_unknown_image_endpoint_uses_default(
        self, mock_prepare, mock_create, mock_get_kwargs
    ):
        """Test that unknown image in endpoint mode passes through without error."""
        mock_init_kwargs = Mock()
        mock_init_kwargs.image_uri = (
            "763104351884.dkr.ecr.us-east-1.amazonaws.com/"
            "vllm:0.17-gpu-py312-cu129-ubuntu22.04-sagemaker-v1"
        )
        mock_init_kwargs.env = {}
        mock_init_kwargs.model_data = "s3://jumpstart-cache/models/vllm/model.tar.gz"
        mock_get_kwargs.return_value = mock_init_kwargs
        ...
        self.assertEqual(result, mock_model)
        self.assertIsNone(builder.model_server)
        self.assertEqual(builder.s3_upload_path, "s3://jumpstart-cache/models/vllm/model.tar.gz")
        mock_create.assert_called_once()

    @patch("sagemaker.core.jumpstart.factory.utils.get_init_kwargs")
    @patch("sagemaker.serve.model_builder.ModelBuilder._create_model")
    @patch("sagemaker.serve.model_builder.ModelBuilder._prepare_for_mode")
    def test_build_for_jumpstart_routes_to_djl(self, mock_prepare, mock_create, mock_get_kwargs):
        ...
        mock_init_kwargs.env = {}
        mock_init_kwargs.model_data = "s3://jumpstart-cache/models/djl/model.tar.gz"
        mock_get_kwargs.return_value = mock_init_kwargs
    ...
    @patch("sagemaker.serve.model_builder.ModelBuilder._create_model")
    @patch("sagemaker.serve.model_builder.ModelBuilder._prepare_for_mode")
    def test_build_for_jumpstart_passes_config_name(
        self, mock_prepare, mock_create, mock_get_kwargs
    ):
        ...
        self.assertEqual(
            call_kwargs.kwargs.get("config_name") or call_kwargs[1].get("config_name"),
            "lmi-optimized",
        )
...
class TestDeployWrappers(unittest.TestCase):
    ...
    def test_djl_deploy_wrapper_sets_timeout(self, mock_deploy):
        ...
        result = builder._djl_model_builder_deploy_wrapper(model_data_download_timeout=1800)
    ...
    def test_js_deploy_wrapper_calls_core_deploy(self, mock_deploy):
        ...
        result = builder._js_builder_deploy_wrapper(endpoint_name="test-endpoint")
