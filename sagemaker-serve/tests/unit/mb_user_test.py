...
def setup_aws_session():
    ...
    try:
        ...
        print(
            "Please run: ada credentials update --account=593793038179 --provider=isengard --role=Admin --once"
        )
        raise
...
def test_basic_build():
    ...
    try:
        ...
        print(
            f"Primary container image: {getattr(core_model.primary_container, 'image', 'Not available')}"
        )
...
def test_build_with_vpc():
    ...
    try:
        ...
        network = Network(
            security_group_ids=["sg-12345678"], subnets=["subnet-12345678", "subnet-87654321"]
        )
...
def test_core_model_operations():
    ...
    try:
        ...
        print(
            f"  - Enable network isolation: {getattr(core_model, 'enable_network_isolation', 'Not available')}"
        )
