...
class TestRunRecommendationJob:
    ...
    def test_enum_inputs_normalized_to_service_strings(
        ...
        kwargs = AIRecommendationJob.create.call_args.kwargs
        assert kwargs["inference_specification"].framework == "VLLM"
        assert kwargs["performance_target"].constraints[0].metric == "throughput"
    ...
    def test_explicit_role_arn_overrides_builder_role(
        ...
        run_recommendation_job(
            ...
            workload=Workload.synthetic(tokenizer="t"),
            performance_target="throughput",
            role_arn="arn:aws:iam::1:role/explicit",
        )
        assert (
            AIRecommendationJob.create.call_args.kwargs["role_arn"]
            == "arn:aws:iam::1:role/explicit"
    ...
    def test_builder_session_used_when_present(self, patch_session, patch_resources):
        _, _, _ = patch_resources
        builder = _builder()
        builder.sagemaker_session = MagicMock()
        builder.sagemaker_session.default_bucket.return_value = "builder-bucket"
        run_recommendation_job(
            builder=builder,
            workload=Workload.synthetic(tokenizer="t"),
            performance_target="throughput",
        )
        builder.sagemaker_session.default_bucket.assert_called()
...
def _client_error(code: str):
    ...
    return ClientError(
        {"Error": {"Code": code, "Message": f"{code} happened"}}, "CreateAIBenchmarkJob"
    )


class TestServiceErrorMapping:
    """Raw service errors are mapped to feature-specific exceptions."""

    def test_access_denied_maps_to_feature_gated(self, patch_session, patch_resources):
        from sagemaker.serve.ai_inference_recommender import FeatureGatedError

        _, AIBenchmarkJob, _ = patch_resources
        AIBenchmarkJob.create.side_effect = _client_error("AccessDeniedException")
        with pytest.raises(FeatureGatedError):
            start_benchmark(endpoint="my-ep", workload=Workload.synthetic(tokenizer="t"))

    def test_validation_maps_to_workload_validation(self, patch_session, patch_resources):
        from sagemaker.serve.ai_inference_recommender import WorkloadValidationError

        _, AIBenchmarkJob, _ = patch_resources
        AIBenchmarkJob.create.side_effect = _client_error("ValidationException")
        with pytest.raises(WorkloadValidationError):
            start_benchmark(endpoint="my-ep", workload=Workload.synthetic(tokenizer="t"))

    def test_recommendation_access_denied_maps_to_feature_gated(
        self, patch_session, patch_resources
    ):
        from sagemaker.serve.ai_inference_recommender import FeatureGatedError
    ...
    def test_unrelated_error_passes_through(self, patch_session, patch_resources):
        _, AIBenchmarkJob, _ = patch_resources
        AIBenchmarkJob.create.side_effect = _client_error("ThrottlingException")
        # Not a gating/validation code -> original error re-raised unchanged.
