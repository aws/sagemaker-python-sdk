...
def _bench_job(uri):
    return BenchmarkJob(
        ...
        ai_benchmark_job_status="Completed",
...
class TestBenchmarkResultFromJob:
    ...
    def test_refreshes_when_status_non_terminal_even_with_output(self):
        # s3_output_location is echoed back at create time, so a non-terminal
        # status must still trigger a refresh (not read stale create-time state).
        job = BenchmarkJob(
            ai_benchmark_job_name="bench-1",
        ...
        def _refresh():
            job.ai_benchmark_job_status = "Completed"
            return job

        with patch.object(BenchmarkJob, "refresh", side_effect=_refresh) as mock_refresh:
            with patch.object(BenchmarkResult, "from_s3", return_value="PARSED"):
                assert BenchmarkResult.from_job(job) == "PARSED"
                mock_refresh.assert_called_once()
...
class TestRecommendationView:
    def test_str_renders_config_and_perf_table(self):
        text = str(_RecommendationView(_fake_recommendation_row(), index=0))
    ...
    def test_repr_is_concise_single_line(self):
        ...
        assert "\n" not in text
        assert text.startswith("Recommendation[0](")
    ...
    def test_handles_missing_optional_fields(self):
        ...
        text = str(_RecommendationView(sparse))
        assert "Recommendation[0]" in text
...
class TestRecommendationsView:
    ...
    def test_str_renders_comparative_table_with_both_rows(self):
        text = str(self._make())
        # Header / framing
    ...
    def test_str_when_empty(self):
        text = str(_RecommendationsView())
        # Should not crash and should clearly indicate emptiness
        assert "Recommendations[0]" in text or "no rows" in text


class TestRecommendationJobShowResult:
    """show_result() returns the same _RecommendationsView (with .best) as
    ModelBuilder.recommendations, not a plain list."""

    def test_show_result_returns_view_with_best(self):
        from sagemaker.serve.ai_inference_recommender.jobs import RecommendationJob

        rows = _fake_two_recommendations()
        ...
        # Same comparative table as mb.recommendations.
        assert "Recommendations[2]" in str(result)
