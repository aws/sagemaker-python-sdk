...
class TestBenchmarkResultRepr:
    ...
    def test_repr_contains_header_and_table_columns(self):
    ...
    def test_repr_lists_well_known_metrics(self):
        text = repr(self._result())
        for metric_name in (
            "request_throughput",
            "request_latency",
    ...
    def test_metrics_repr_independently_renders_table(self):
        text = repr(self._result().metrics)
        assert "request_throughput" in text
        assert "request_latency" in text
        # BenchmarkMetrics.__repr__ is a bare table — no wrapping header.
...
class TestBenchmarkResultMetadataFields:
    ...
    def test_repr_shows_endpoint_workload_and_tool_version(self):
        text = repr(self._result())
        assert "my-endpoint" in text
        assert "my-wl-cfg" in text
        assert "0.6.0" in text

    def test_repr_renders_dashes_when_metadata_missing(self):
        text = repr(
