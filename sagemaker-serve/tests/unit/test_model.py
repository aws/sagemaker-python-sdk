...
_src_dir = _current_dir.parent.parent / "src"
...
ModelBase = local_model.ModelBase
_Model = local_model._Model
FrameworkModel = local_model.FrameworkModel
ModelPackage = local_model.ModelPackage
...
from sagemaker.config import (EDGE_PACKAGING_JOB, KEY, KMS_KEY_ID,
                              OUTPUT_CONFIG, RESOURCE_KEY, ROLE_ARN, SAGEMAKER,
                              SCHEMA_VERSION, TAGS, VALIDATION_SPECIFICATION,
                              VALUE)
from sagemaker.enums import EndpointType
from sagemaker.huggingface.model import HuggingFaceModel
from sagemaker.jumpstart.constants import (
    JUMPSTART_GATED_AND_PUBLIC_BUCKET_NAME_SET, JUMPSTART_RESOURCE_BASE_NAME)
...
def test_prepare_container_def_with_accept_eula(
    ...
    env = {"FOO": "BAR"}
    model = _Model(MODEL_IMAGE, MODEL_DATA, env=env, role=ROLE)
...
def test_prepare_container_def_with_model_data_and_env_s3_gzip():
    env = {"FOO": "BAR"}
    model = _Model(MODEL_IMAGE, MODEL_DATA, env=env)
...
def test_prepare_container_def_with_image_config():
    ...
    model = _Model(MODEL_IMAGE, image_config=image_config)
...
def test_model_enable_network_isolation():
    model = _Model(MODEL_IMAGE, MODEL_DATA)
    assert model.enable_network_isolation() is False

    model = _Model(MODEL_IMAGE, MODEL_DATA, enable_network_isolation=True)
...
def test_create_sagemaker_model(prepare_container_def, sagemaker_session):
    ...
    model = _Model(MODEL_DATA, MODEL_IMAGE, name=MODEL_NAME, sagemaker_session=sagemaker_session)
...
def test_create_sagemaker_model_accelerator_type(prepare_container_def, sagemaker_session):
    model = _Model(MODEL_IMAGE, MODEL_DATA, name=MODEL_NAME, sagemaker_session=sagemaker_session)
...
def test_create_sagemaker_model_generates_model_name(
    ...
    base_name_from_image.assert_called_with(MODEL_IMAGE, default_base_name="_Model")
...
def test_create_sagemaker_model_generates_model_name_each_time(
    ...
    # base_name_from_image.assert_called_once_with(MODEL_IMAGE, default_base_name="_Model")
    name_from_base.assert_called_with(base_name_from_image.return_value)
...
def test_model_create_transformer_optional_params(create_sagemaker_model, sagemaker_session):
    model = _Model(MODEL_IMAGE, MODEL_DATA, sagemaker_session=sagemaker_session)
...
def test_transformer_creates_correct_session(local_session, session):
    local_session.return_value.sagemaker_config = {}
    session.return_value.sagemaker_config = {}
    model = _Model(MODEL_IMAGE, MODEL_DATA, sagemaker_session=None)
