...
class TestModelBuilderV3Deploy(unittest.TestCase):
    ...
    def test_deploy_with_serverless_config(self, mock_deploy):
        ...
        result = builder.deploy(
            endpoint_name="test-endpoint", inference_config=serverless_config, wait=False
        )
    ...
    def test_deploy_with_async_config(self, mock_deploy):
        ...
        async_config = AsyncInferenceConfig(
            output_path="s3://bucket/output", max_concurrent_invocations_per_instance=5
        )
        ...
        result = builder.deploy(
            endpoint_name="test-endpoint", inference_config=async_config, wait=False
        )
    ...
    def test_deploy_with_update_endpoint(self, mock_deploy):
        ...
        result = builder.deploy(endpoint_name="existing-endpoint", update_endpoint=True, wait=False)
...
class TestModelBuilderV3BuildSingleModelBuilder(unittest.TestCase):
    ...
    def test_build_single_with_explicit_model_server(
        self, mock_mlflow, mock_validations, mock_serve_setting, mock_translators, mock_build_server
    ):
...
class TestModelBuilderV3Validations(unittest.TestCase):
    ...
    def test_validation_model_and_inference_spec_mutually_exclusive(self):
        ...
        self.assertIn(
            "Can only set one of the following: model, inference_spec", str(context.exception)
        )
    ...
    def test_validation_custom_image_requires_model_server(self):
        ...
        self.assertIn(
            "Model_server must be set when non-first-party image_uri is set", str(context.exception)
        )
...
class TestModelBuilderV3ModelServers(unittest.TestCase):
    ...
    def test_build_with_triton(
        self, mock_mlflow, mock_validations, mock_serve_setting, mock_translators, mock_build_triton
    ):
    ...
    def test_build_with_djl(
        self, mock_mlflow, mock_validations, mock_serve_setting, mock_translators, mock_build_djl
    ):
    ...
    def test_build_with_vllm_for_text_generation(
        ...
        builder = ModelBuilder(
            model="gpt2", role_arn=self.mock_role_arn, sagemaker_session=self.mock_session
        )
