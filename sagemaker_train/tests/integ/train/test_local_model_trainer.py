...
CWD = os.getcwd()
SOURCE_DIR = os.path.join(DATA_DIR, "local_script")
...
def test_single_container_local_mode_local_data(sagemaker_session):
...
def test_single_container_local_mode_s3_data(sagemaker_session):
    with lock.lock(LOCK_PATH):
        try:
...
def test_multi_container_local_mode(sagemaker_session):
    with lock.lock(LOCK_PATH):
        try:
            source_code = SourceCode(
                source_dir=SOURCE_DIR,
