...
def test_git_support_with_mxnet(sagemaker_local_session):
    ...
    mx = MXNet(entry_point=script_path, role='SageMakerRole',
               source_dir=source_dir, dependencies=dependencies,
    ...
    with lock.lock(LOCK_PATH):
        try:
            ...
            model = MXNetModel(model_data, 'SageMakerRole', entry_point=script_path,
                               ...
                               framework_version=MXNet.LATEST_VERSION, git_config=git_config)
...
def test_git_support_with_sklearn(sagemaker_local_session, sklearn_full_version):
    ...
    git_config = {'repo': 'https://github.com/GaryTu1020/python-sdk-testing.git',
                  'branch': 'branch1', 'commit': 'aafa4e96237dd78a015d5df22bfcfef46845c3c5'}
    source_dir = 'sklearn'
    sklearn = SKLearn(entry_point=script_path, role='SageMakerRole',
                      source_dir=source_dir, py_version=PYTHON_VERSION, train_instance_count=1,
                      train_instance_type='local', sagemaker_session=sagemaker_local_session,
                      framework_version=sklearn_full_version, hyperparameters={'epochs': 1},
    ...
    with lock.lock(LOCK_PATH):
        try:
            ...
            model_data = desc['ModelArtifacts']['S3ModelArtifacts']
            model = SKLearnModel(model_data, 'SageMakerRole', entry_point=script_path, source_dir=source_dir,
                                 sagemaker_session=sagemaker_local_session, git_config=git_config)
            predictor = model.deploy(1, 'local')
