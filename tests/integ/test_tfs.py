...
def tfs_predictor_with_model_and_entry_point_same_tar(
    instance_type, sagemaker_session, tf_full_version, tmpdir
):
    endpoint_name = sagemaker.utils.unique_name_from_base("sagemaker-tensorflow-serving")
    ...
    model_data = sagemaker_session.upload_data(
        path=model_tar, key_prefix="tensorflow-serving/models"
    )
...
def tfs_predictor_with_model_and_entry_point_and_dependencies(
    instance_type, sagemaker_session, tf_full_version
):
    endpoint_name = sagemaker.utils.unique_name_from_base("sagemaker-tensorflow-serving")
    ...
    with tests.integ.timeout.timeout_and_delete_endpoint_by_name(endpoint_name, sagemaker_session):
        ...
        dependencies = [
            os.path.join(
                tests.integ.DATA_DIR, "tfs/tfs-test-entrypoint-and-dependencies/dependency.py"
            )
        ]
