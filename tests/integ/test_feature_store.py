    ...
    df = pd.DataFrame(
        {
            ...
            "feature9": pd.Series([[1.1, 2.3], [1.4, 25, 3.2], [1.0, 3, 4], [1.2], []], dtype="object"),
            "feature10": pd.Series([["a", "abc"], ["b", "c"], ["c", "None"], ["d"], []], dtype="object"),
        }
    )
    return df
...
def test_create_feature_store_ingest_with_offline_target_stores(
    ...
    with cleanup_feature_group(feature_group):
        ...
        for index, value in pandas_data_frame["feature1"].items():
            assert feature_group.get_record(
                record_identifier_value_as_string=str(value)
            ) is None
...
def test_ingest_in_memory_multi_process_with_collection_types(
    ...
    feature_group.load_feature_definitions(data_frame=pandas_data_frame_with_collection_type,
    ...
    with (cleanup_feature_group(feature_group)):
        ...
        ingestion_manager = feature_group.ingest(
            data_frame=pandas_data_frame_with_collection_type, max_workers=3, max_processes=2, wait=True
        )
        ingestion_manager.wait()
        assert 0 == len(ingestion_manager.failed_rows)
        assert output["FeatureGroupArn"].endswith(f"feature-group/{feature_group_name}")
        ...
        new_row_data = [
            10.0, 10, "2020-10-30T03:43:21Z", 5.0, ["a", "b"], [1, 2, None], [3.0, 4.0], [1, 2], [3.0, 4.0], ["a", "b"]
        ]
        pandas_data_frame_with_collection_type.loc[len(pandas_data_frame_with_collection_type)] = new_row_data
        with pytest.raises(IngestionError) as error:
            feature_group.ingest(
                data_frame=pandas_data_frame_with_collection_type, max_workers=1, max_processes=1, wait=True
            )
