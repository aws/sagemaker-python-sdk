...
class JumpStartCuratedPublicHubTest(unittest.TestCase):
    ...
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.get_studio_model_metadata_map_from_region")
    ...
    def test_get_curated_hub_and_curated_hub_s3_bucket_names_hub_does_not_exist_uses_input_values(self, mock_get_preexisting):
        ...
        res_hub_name, res_hub_bucket_name = self.test_curated_hub._get_curated_hub_and_curated_hub_s3_bucket_names(
    ...
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._get_preexisting_hub_and_s3_bucket_names")
    def test_get_curated_hub_and_curated_hub_s3_bucket_names_hub_does_exist_uses_preexisting_values(self, mock_get_preexisting):
        mock_get_preexisting.return_value = (self.test_preexisting_hub_name, self.test_preexisting_bucket_name)
    ...
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._get_preexisting_hub_and_s3_bucket_names")
    def test_get_curated_hub_and_curated_hub_s3_bucket_names_hub_does_exist_import_to_existing_hub_false_throws_error(self, mock_get_preexisting):
        mock_get_preexisting.return_value = (self.test_preexisting_hub_name, self.test_preexisting_bucket_name)
        ...
        error_msg = f"Hub with name {self.test_preexisting_hub_name} detected on account. The limit of hubs per account is 1. If you wish to use this hub as the curated hub, please set the flag `import_to_preexisting_hub` to True."
        self.assertEqual(error_msg, str(context.exception))


    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._create_hub_and_hub_bucket")
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._should_skip_create")
    def test_create_if_skip_create_true_does_not_create(self, mock_skip_create, mock_create_hub_and_hub_bucket):
    ...
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._create_hub_and_hub_bucket")
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._should_skip_create")
    def test_create_if_skip_create_false_does_create(self, mock_skip_create, mock_create_hub_and_hub_bucket):
    ...
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._get_model_specs")
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._import_models")
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._model_needs_update")
    def test_sync_filters_models_that_dont_need_update(self, mock_need_update, mock_import_models, mock_model_specs):
    ...
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._import_models")
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._model_needs_update")
    def test_sync_force_update_true_updates_all_models(self, mock_need_update, test_import_models, mock_model_specs):
        test_list = ["test_specs_1", "test_specs_2"]
        mock_model_specs.return_value = test_list
        mock_need_update.side_effect = self._mock_should_update_model
    ...
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._delete_model_dependencies_no_content_noop")
    def test_delete_model_from_curated_hub_deletes_dependencies_true_deletes_dependecnies(self, mock_delete_model_deps):
    ...
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._delete_model_dependencies_no_content_noop")
    def test_delete_model_from_curated_hub_deletes_dependencies_false_keeps_dependecnies(self, mock_delete_model_deps):
        mock_hub_client = Mock()
        self.test_curated_hub._hub_client = mock_hub_client
    ...
    def test_format_dependency_dst_uris_for_delete_objects_if_dict_append_dict_keys(self, mock_find_objects):
        test_dependency = Dependency(
            DependencyOriginPath="s3://test_origin",
    ...
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._format_dependency_dst_uris_for_delete_objects")
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._get_hub_content_dependencies_from_model_document")
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._delete_model_dependencies_no_content_noop")
    def test_delete_model_dependencies_no_content_noop_delete_error_throws_error(self, mock_delete_model_deps, mock_get_deps, mock_format_deps):
        mock_s3_client = Mock()
    ...
    @patch("sagemaker.jumpstart.curated_hub.jumpstart_curated_public_hub.JumpStartCuratedPublicHub._format_dependency_dst_uris_for_delete_objects")
    def test_delete_model_dependencies_no_content_noop_delete_error_throws_error(self, mock_format_deps):
        mock_s3_client = Mock()
        self.test_curated_hub._s3_client = mock_s3_client
        mock_format_deps.return_value = []
        mock_s3_client.delete_objects.return_value = {
        ...
        self.assertRaises(Exception, self.test_curated_hub._delete_model_dependencies_no_content_noop(test_spec))
