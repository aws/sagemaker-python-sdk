...
def test_get_gpu_info_success(session):
    client = session.boto_session.client("ec2")
    ...
    instance_gpu_info = hardware_detector._get_gpu_info(VALID_INSTANCE_TYPE, session)
...
@patch("sagemaker.session.Session")
def test_get_gpu_info_describe_instance_types_throws(session):
    client = session.boto_session.client("ec2")
...
def test_get_gpu_info_fallback_success():
    fallback_instance_gpu_info = hardware_detector._get_gpu_info_fallback(VALID_INSTANCE_TYPE)
