...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "source_code_config": SourceCodeConfig(source_dir="test-data", entry_script="train.sh"),
            "expected_command": "chmod +x train.sh && bash train.sh",
            "should_throw": False,
        },
        {
            "source_code_config": SourceCodeConfig(
                source_dir="test-data",
                entry_script="train.ts",
            ),
            "should_throw": True,
        },
        {
...
def test_get_script_mode_command(model_trainer, test_case):
    if test_case["should_throw"]:
        with pytest.raises((ValueError, AssertionError)):
            model_trainer._get_script_mode_command(test_case["source_code_config"])
    else:
        command = model_trainer._get_script_mode_command(test_case["source_code_config"])
