...
def test_build_model_package_group_asset(boto_session, sagemaker_client):
    mpg_arn = "arn:aws:sagemaker:us-east-2:1234567890:model-package-group/my-mpg"
    mpg_name = "my-mpg"
    mpg_asset_builder = ModelPackageGroupAssetBuilder(boto_session)
    ...
    sagemaker_client.describe_model_package_group.return_value = describe_resp
...
def test_build_feature_group_asset(
    ...
    fg_asset_builder = FeatureGroupAssetBuilder(
        boto_session, domain_id, owning_project_id, environment_id
    )
    now = datetime.datetime.now()
    ...
    fg_forms = {
        ...
        "offlineStoreStatus": describe_resp["OfflineStoreStatus"],
        "featureDefinitions": [
            {
                "featureName": "feature_1",
