...
@pytest.fixture
def sagemaker_session(sagemaker_client, datazone_client, lf_client):
...
@pytest.fixture
def asset_manager(sagemaker_session, domain_id, owning_project_id, environment_id):
    return AssetManager(
...
def test_asset_manager_export_pipeline_asset_to_inventory(
    asset_manager: AssetManager,
    datazone_client: Mock,
):
    pipeline_arn = "arn:aws:sagemaker:us-east-2:1234567890:pipeline/my-pipeline"
    pipeline_asset = Asset(
        name="asset-name",
        type_id=AssetTypeIdentifier.SageMakerPipelineAssetType,
        external_id=pipeline_arn,
        forms_input=[
            {
                "pipelineName": "my-pipeline",
                "pipelineDisplayName": "my-pipeline",
                "pipelineArn": pipeline_arn,
                "pipelineDescription": "my good pipeline",
    ...
    with patch.object(
        PipelineAssetBuilder, "build_asset", return_value=pipeline_asset
    ) as mock_method:
        asset_manager.export_to_inventory(
            resource_arn=pipeline_arn,
            description="my description",
            glossary_terms=["term1", "term2"],
        )
        mock_method.assert_called_once_with(pipeline_arn)
        datazone_client.create_asset.assert_called_with(
            domainIdentifier=asset_manager.domain_id,
            name=pipeline_asset.name,
            owningProjectIdentifier=asset_manager.owning_project_id,
            typeIdentifier=pipeline_asset.type_id.value,
            externalIdentifier=pipeline_asset.external_id,
            formsInput=pipeline_asset.forms_input,
            description="my description",
            glossaryTerms=["term1", "term2"],
        )
        assert pipeline_asset.id == "asset-id"

    pipeline_asset_2 = Asset(
        name="asset-name-2",
        type_id=AssetTypeIdentifier.SageMakerPipelineAssetType,
        external_id=pipeline_arn + "-2",
        forms_input=[
            {
                "pipelineName": "my-pipeline-2",
                "pipelineDisplayName": "my-pipeline",
                "pipelineArn": pipeline_arn + "-2",
                "pipelineDescription": "my good pipeline",
                "creationTime": "2022-01-01T00:00:00Z",
                "lastModifiedTime": "2022-01-01T00:00:00Z",
            }
    ...
    pipeline = Pipeline(name="my-pipeline-2")

    with patch.object(
        PipelineAssetBuilder, "build_asset", return_value=pipeline_asset_2
    ) as mock_method:
        asset_manager.export_to_inventory(
            resource=pipeline, description="my description 2", glossary_terms=["term1", "term2"]
        )
        mock_method.assert_called_once_with(pipeline)
        datazone_client.create_asset.assert_called_with(
            domainIdentifier=asset_manager.domain_id,
            name=pipeline_asset_2.name,
            owningProjectIdentifier=asset_manager.owning_project_id,
            typeIdentifier=pipeline_asset_2.type_id.value,
            externalIdentifier=pipeline_asset_2.external_id,
            formsInput=pipeline_asset_2.forms_input,
            description="my description 2",
            glossaryTerms=["term1", "term2"],
        )
        assert pipeline_asset_2.id == "asset-id"
...
def test_asset_manager_export_mpg_asset_to_inventory(
    ...
    mpg = ModelPackageGroup(
        name="my-pipeline-2", description="my good mpg", sagemaker_session=sagemaker_session
    )
...
def test_asset_manager_export_fg_asset_to_inventory(
    ...
    fg_offline_asset = Asset(
        ...
        type_id=AssetTypeIdentifier.SageMakerFeatureGroupAssetType,
        external_id=fg_arn,
        forms_input=[{"glueTableName": "my-glue-table"}],
    )
...
def test_export_inventory_existing_asset_create_revision(asset_manager, datazone_client):
    pipeline_arn = "arn:aws:sagemaker:us-east-2:1234567890:pipeline/my-pipeline"
    pipeline_asset = Asset(
        name="asset-name",
        type_id=AssetTypeIdentifier.SageMakerPipelineAssetType,
    ...
    asset_id = "asset-id"
    datazone_client.create_asset = MagicMock(
        name="create-asset", side_effect=ValueError("ConflictException")
    )
    datazone_client.search.return_value = {"items": [{"assetItem": {"identifier": asset_id}}]}

    with patch.object(
        PipelineAssetBuilder, "build_asset", return_value=pipeline_asset
    ) as mock_method:
        asset_manager.export_to_inventory(resource_arn=pipeline_arn)
        mock_method.assert_called_once_with(pipeline_arn)
        datazone_client.create_asset.assert_called_with(
            domainIdentifier=asset_manager.domain_id,
            name=pipeline_asset.name,
        ...
        datazone_client.search.assert_called_once_with(
            ...
            filters={
                "filter": {
                    "attribute": "SageMakerPipelineForm.pipelineArn",
                    "value": pipeline_asset.external_id,
                }
            },
        )
...
def test_publish_to_catalog(asset_manager, datazone_client):
    ...
    listing: Listing = asset_manager.publish_to_catalog(pipeline_asset)

    datazone_client.create_listing_change_set.assert_called_once_with(
        action=ChangeSetAction.PUBLISH,
        domainIdentifier=asset_manager.domain_id,
        entityIdentifier=pipeline_asset.id,
        entityType="ASSET",
    )
...
def test_un_publish_to_catalog(asset_manager, datazone_client):
    ...
    listing: Listing = asset_manager.remove_from_catalog(pipeline_asset)

    datazone_client.create_listing_change_set.assert_called_once_with(
        action=ChangeSetAction.UNPUBLISH,
        domainIdentifier=asset_manager.domain_id,
        entityIdentifier=pipeline_asset.id,
        entityType="ASSET",
    )
