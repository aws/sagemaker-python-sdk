...
def _sklearn_estimator(sagemaker_session, framework_version=defaults.SKLEARN_VERSION, train_instance_type=None,
...
def test_train_image(sagemaker_session, sklearn_version):
    ...
    sklearn = SKLearn(entry_point=SCRIPT_PATH, role=ROLE, sagemaker_session=sagemaker_session,
                      train_instance_type=INSTANCE_TYPE, framework_version=sklearn_version,
                      container_log_level=container_log_level, py_version=PYTHON_VERSION,
...
def test_create_model_with_optional_params(sagemaker_session):
    ...
    sklearn = SKLearn(entry_point=SCRIPT_PATH, role=ROLE, sagemaker_session=sagemaker_session,
                      train_instance_type=INSTANCE_TYPE, container_log_level=container_log_level,
    ...
    model = sklearn.create_model(role=new_role, model_server_workers=model_server_workers,
...
def test_sklearn(strftime, sagemaker_session, sklearn_version):
    sklearn = SKLearn(entry_point=SCRIPT_PATH, role=ROLE, sagemaker_session=sagemaker_session,
                      train_instance_type=INSTANCE_TYPE, py_version=PYTHON_VERSION, framework_version=sklearn_version)
    ...
    assert {'Environment':
            {'SAGEMAKER_SUBMIT_DIRECTORY':
             's3://mybucket/sagemaker-scikit-learn-{}/source/sourcedir.tar.gz'.format(TIMESTAMP),
             'SAGEMAKER_PROGRAM': 'dummy_script.py',
             'SAGEMAKER_ENABLE_CLOUDWATCH_METRICS': 'false',
             'SAGEMAKER_REGION': 'us-west-2',
             'SAGEMAKER_CONTAINER_LOG_LEVEL': '20'},
            'Image': expected_image_base.format(sklearn_version, PYTHON_VERSION),
            'ModelDataUrl': 's3://m/m.tar.gz'} == model.prepare_container_def(CPU)
...
def test_train_image_default(sagemaker_session):
    sklearn = SKLearn(entry_point=SCRIPT_PATH, role=ROLE, sagemaker_session=sagemaker_session,
                      train_instance_type=INSTANCE_TYPE, py_version=PYTHON_VERSION)
...
def test_train_image_cpu_instances(sagemaker_session, sklearn_version):
    sklearn = _sklearn_estimator(sagemaker_session, sklearn_version, train_instance_type='ml.c2.2xlarge')
    assert sklearn.train_image() == _get_full_cpu_image_uri(sklearn_version)

    sklearn = _sklearn_estimator(sagemaker_session, sklearn_version, train_instance_type='ml.c4.2xlarge')
    assert sklearn.train_image() == _get_full_cpu_image_uri(sklearn_version)
...
def test_attach(sagemaker_session, sklearn_version):
    training_image = '1.dkr.ecr.us-west-2.amazonaws.com/sagemaker-scikit-learn:{}-cpu-{}'.format(sklearn_version,
                                                                                                 PYTHON_VERSION)
