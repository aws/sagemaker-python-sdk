...
def test_tf_cpu_images(sagemaker_session, tf_version):
...
def test_tf_gpu_images(sagemaker_session, tf_version):
...
def test_train_image_default(sagemaker_session):
    ...
    assert _get_full_cpu_image_uri(defaults.TF_VERSION, repo=SM_IMAGE_REPO_NAME) == tf.train_image()
...
def test_legacy_mode_framework_name(sagemaker_session):
    tf = _build_tf(sagemaker_session=sagemaker_session, framework_version="1.10")
