    ...
    **pandas_read_csv_kwargs,
    ...
    if latest_ingestion:
        if event_time_feature_name is not None:
            query += str(
                f"AND {event_time_feature_name}=(SELECT MAX({event_time_feature_name}) FROM "
...
def prepare_fg_from_dataframe_or_file(
    ...
    **pandas_read_csv_kwargs,
    ...
    if isinstance(dataframe_or_path, DataFrame):
        ...
        exc = Exception(
            str(
                f"Invalid type {type(dataframe_or_path)} for argument dataframe_or_path."
    ...
    if lg_id != lg_uniq:
        exc = Exception(
            str(
                f"Record identifier {record_id} have {abs(lg_id - lg_uniq)} duplicated rows."
