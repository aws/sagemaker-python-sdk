...
class ModelPackage(Model):
    ...
    def remove_customer_metadata_properties(self, customer_metadata_properties_to_remove: List[str]):
    ...
    def add_inference_specification(
        ...
        if containers is not None and image_uris is not None:
             raise ValueError(
                "Cannot have both containers and image_uris."
            )
        if containers is None and image_uris is None:
             raise ValueError(
                "Should have either containers or image_uris for inference."
            )
        container_def = []
        if image_uris: 
