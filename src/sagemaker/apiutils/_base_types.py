...
class Record(ApiObject):
    ...
    # Lineage entity creates that a pipeline step can compose rather than execute.
    # Requests for these are routed through the session's
    # ``_intercept_create_request`` seam, so a ``PipelineSession`` captures them as
    # step arguments while a plain ``Session`` calls the service. Used by
    # ``sagemaker.workflow.lineage_step.LineageStep``.
