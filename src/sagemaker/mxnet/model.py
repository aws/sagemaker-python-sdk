...
class MXNetModel(FrameworkModel):
    ...
    def _is_mms_version(self):
        """Whether the framework version corresponds to an inference image using the MMS Server.

        MMS (Multi-Model) Server: (https://github.com/awslabs/multi-model-server).
