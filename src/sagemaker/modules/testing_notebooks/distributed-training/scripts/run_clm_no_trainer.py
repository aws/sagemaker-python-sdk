...
def parse_arge():
    ...
    parser.add_argument(
        "--model_id",
        type=str,
        default=os.environ.get("SM_HP_MODEL_ID", "google/flan-t5-xl"),
        help="Model id to use for training.",
    )
    parser.add_argument(
        "--dataset_path",
        type=str,
        default=os.environ.get("SM_CHANNEL_DATASET", "lm_dataset"),
    ...
    parser.add_argument(
        "--epochs",
        type=int,
        default=os.environ.get("SM_HP_EPOCHS", 1),
        help="Number of epochs to train for.",
    )
    parser.add_argument(
        "--max_steps",
        type=int,
        default=os.environ.get("SM_HP_MAX_STEPS", None),
        help="Number of epochs to train for.",
    )
    parser.add_argument(
        "--per_device_train_batch_size",
        type=int,
        default=os.environ.get("SM_HP_PER_DEVICE_TRAIN_BATCH_SIZE", 1),
        help="Batch size to use for training.",
    )
    parser.add_argument(
        "--lr",
        type=float,
        default=os.environ.get("SM_HP_LR", 3e-5),
        help="Learning rate to use for training.",
    )
    parser.add_argument(
        "--optimizer",
        type=str,
        default=os.environ.get("SM_HP_OPTIMIZER", "adamw_hf"),
        help="Learning rate to use for training.",
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=os.environ.get("SM_HP_SEED", 42),
        help="Seed to use for training.",
    )
    parser.add_argument(
        "--access_token",
        type=str,
        default=os.environ.get("SM_HP_ACCESS_TOKEN", None),
    )
    parser.add_argument(
        "--num_train_epochs",
        type=int,
        default=os.environ.get("SM_HP_NUM_TRAIN_EPOCHS", 1),
    ...
    parser.add_argument(
        ...
        type=bool,
        default=os.environ.get("SM_HP_GRADIENT_CHECKPOINTING", True),
    ...
    parser.add_argument(
        "--fsdp", type=str, default=os.environ.get("SM_HP_FSDP", None), help="Whether to use fsdp."
    )
    parser.add_argument(
        "--fsdp_transformer_layer_cls_to_wrap",
        type=str,
        default=os.environ.get("SM_HP_FSDP_TRANSFORMER_LAYER_CLS_TO_WRAP", None),
        help="Which transformer layer to wrap with fsdp.",
    )
    parser.add_argument(
        "--max_train_steps",
        type=int,
        default=os.environ.get("SM_HP_MAX_TRAIN_STEPS", None),
        help="Total number of training steps to perform. If provided, overrides num_train_epochs.",
    )
    parser.add_argument(
        "--learning_rate",
        type=float,
        default=os.environ.get("SM_HP_LEARNING_RATE", 5e-5),
        help="Initial learning rate (after the potential warmup period) to use.",
    )
    parser.add_argument(
        "--gradient_accumulation_steps",
        type=int,
        default=os.environ.get("SM_HP_GRADIENT_ACCUMULATION_STEPS", 1),
        help="Number of updates steps to accumulate before performing a backward/update pass.",
    )
    parser.add_argument(
        "--lr_scheduler_type",
        type=SchedulerType,
        default=os.environ.get("SM_HP_LR_SCHEDULER_TYPE", "linear"),
    ...
    parser.add_argument(
        "--num_warmup_steps",
        type=int,
        default=os.environ.get("SM_HP_NUM_WARMUP_STEPS", 0),
        help="Number of steps for the warmup in the lr scheduler.",
    )
    parser.add_argument(
        "--limit_all_gathers",
        type=bool,
        default=os.environ.get("SM_HP_LIMIT_ALL_GATHERS", False),
    )
    parser.add_argument(
        "--forward_prefetch",
        type=bool,
        default=os.environ.get("SM_HP_FORWARD_PREFETCH", False),
    )
    parser.add_argument(
        "--weight_decay",
        type=float,
        default=os.environ.get("SM_HP_WEIGHT_DECAY", 0.0),
    ...
    parser.add_argument(
        "--cache_dir",
        type=str,
        default=os.environ.get("SM_HP_CACHE_DIR", None),
    )
