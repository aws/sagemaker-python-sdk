...
def to_pipeline(
    ...
    _validate_input_for_to_pipeline_api(step)

    remote_decorator_config = _get_remote_decorator_config_from_input(
        wrapped_func=step, sagemaker_session=sagemaker_session
    )
    _role = role or get_execution_role(sagemaker_session)
    ...
    return pipeline.describe()["PipelineArn"]
...
def schedule(
    ...
    event_bridge_scheduler_helper = EventBridgeSchedulerHelper(
        sagemaker_session,
        sagemaker_session.boto_session.client("scheduler"),
    )
    describe_pipeline_response = sagemaker_session.sagemaker_client.describe_pipeline(
    ...
    return event_bridge_schedule_arn
...
def execute(
    ...
    """Starts an execution of a SageMaker Pipeline created by FeatureProcessor
    ...
    start_pipeline_execution_request = dict(
        ...
        PipelineParameters=[
            dict(
                ...
                Value=execution_time.strftime(EXECUTION_TIME_PIPELINE_PARAMETER_FORMAT),
            )
        ],
    )
    logger.info("Starting an execution for pipline %s", pipeline_name)
    execution_response = sagemaker_session.sagemaker_client.start_pipeline_execution(
        **start_pipeline_execution_request
    )
    execution_arn = execution_response["PipelineExecutionArn"]
    logger.info(
        "Execution %s for pipeline %s is successfully started.", execution_arn, pipeline_name
    )
    return execution_arn


def delete_schedule(pipeline_name: str, sagemaker_session: Session = Session()) -> None:
...
def describe(
    pipeline_name: str, sagemaker_session: Session = Session()
    ...
    try:
        ...
        if "RetryStrategy" in pipeline_step["Arguments"]:
            describe_response_dict["max_retries"] = pipeline_step["Arguments"]["RetryStrategy"][
...
def list_schedules(sagemaker_session: Session = Session()) -> List[str]:
    ...
    Args:
        sagemaker_session (Session): Session object which manages interactions
                ...
                function creates one using the default AWS configuration chain.
    Returns:
        Dict[str]: Return list of SageMaker Pipeline ARNs created for feature processor.
    """
    next_token = None
    pipeline_arns = []
    while True:
        ...
        list_contexts_response = sagemaker_session.sagemaker_client.list_contexts(
            **list_contexts_request
        )
        for context in list_contexts_response["ContextSummaries"]:
            pipeline_arns.append(context["Source"]["SourceUri"])
...
def _validate_input_for_to_pipeline_api(step: Callable) -> None:
...
def _get_container_entry_point_and_arguments(
    ...
    return dict(container_entry_point=jobs_container_entrypoint, container_arguments=container_args)
...
def _get_remote_decorator_config_from_input(
    ...
    remote_decorator_config = getattr(
        wrapped_func,
        "job_settings",
        _get_default_job_settings_with_spark_config(sagemaker_session),
    )
    # TODO: Remove this after GA
    remote_decorator_config.python_sdk_whl_s3_uri = SAGEMAKER_WHL_FILE_S3_PATH
...
def _get_default_job_settings_with_spark_config(sagemaker_session: Session) -> _JobSettings:
    """Generate default remote decorator job settings for training step configuration.

    This default setting assumes that feature processor will be used in Spark mode.
    ...
    Args:
        sagemaker_session (Session): Session object which manages interactions
                ...
                function creates one using the default AWS configuration chain.
    Returns:
        _JobSettings: Return default remote decorator job settings for training step configuration
            for feature processor.
    """
