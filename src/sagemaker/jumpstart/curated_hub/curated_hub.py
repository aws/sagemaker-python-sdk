...
class CuratedHub:
    ...
    def _sync_public_model_to_hub(self, model: JumpStartModelInfo, thread_num: int):
        ...
        search_keywords = [
            f"{JUMPSTART_HUB_MODEL_ID_TAG_PREFIX}:{model.model_id.replace('-', '')}",
            f"{JUMPSTART_HUB_MODEL_VERSION_TAG_PREFIX}:{model.version.replace('.', '')}",
            f"{FRAMEWORK_TAG_PREFIX}:{model_specs.get_framework()}".lower(),
            f"{TASK_TAG_PREFIX}:TODOpullfromspecs".lower(),
        ]
