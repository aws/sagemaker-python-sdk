...
from sagemaker.ai_registry.air_constants import (
    ...
    EVALUATOR_HUB_CONTENT_SUBTYPE,
    HubContentStatus,
    EVALUATOR_DEFAULT_S3_PREFIX,
    EVALUATOR_DEFAULT_RUNTIME,
...
class Evaluator(AIRHubEntity):
    ...
    def get(cls, name: str, sagemaker_session=None) -> "Evaluator":
        ...
        response = AIRHub.describe_hub_content(
            EVALUATOR_HUB_CONTENT_TYPE, name, session=sagemaker_session
        )
        doc = json.loads(response[RESPONSE_KEY_HUB_CONTENT_DOCUMENT])
        try:
            keywords = {
    ...
    def create(
        ...
        hub_content_document = json.dumps(
            {"SubType": EVALUATOR_HUB_CONTENT_SUBTYPE, "JsonContent": json.dumps(json_content)}
        )
        ...
        if source.startswith("s3://"):
            ...
            try:
                return AIRHub.upload_to_s3(
                    _get_default_bucket(), f"{EVALUATOR_DEFAULT_S3_PREFIX}/{name}", source
                )
            except Exception as e:
                raise ValueError(
                    f"[PySDK Error] Failed to upload prompt source to S3: {str(e)}"
                ) from e

    @classmethod
    def _handle_reward_function(
        cls, name: str, source: Optional[str], role: Optional[str]
    ) -> tuple[EvaluatorMethod, str]:
    ...
    def _create_lambda_function(
        cls, name: str, source_file: str, role: Optional[str]
    ) -> tuple[EvaluatorMethod, str]:
        ...
        AIRHub.upload_to_s3(
            _get_default_bucket(), f"{EVALUATOR_DEFAULT_S3_PREFIX}/{name}", source_file
        )
    ...
    def get_all(
        cls, type: Optional[str] = None, max_results: Optional[int] = None, sagemaker_session=None
    ):
