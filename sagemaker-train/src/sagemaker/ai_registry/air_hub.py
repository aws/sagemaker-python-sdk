...
class AIRHub:
    ...
    def list_hub_content(
        ...
        for summary in summaries:
            ...
            detailed_response = cls.describe_hub_content(
                hub_content_type, hub_content_name, session=session
            )
            items.append(detailed_response)

        return {"items": items, "next_token": response.get("NextToken")}

    @classmethod
    @_telemetry_emitter(
        feature=Feature.MODEL_CUSTOMIZATION, func_name="AIRHub.describe_hub_content"
    )
    ...
    @classmethod
    @_telemetry_emitter(
        feature=Feature.MODEL_CUSTOMIZATION, func_name="AIRHub.list_hub_content_versions"
    )
    def list_hub_content_versions(
        cls, hub_content_type: str, hub_content_name: str, session: Optional[Session] = None
    ):
    ...
    def upload_to_s3(bucket: str, prefix: str, local_file_path: str) -> str:
        ...
        AIRHub._s3_client.upload_file(local_file_path, bucket, prefix, ExtraArgs=extra_args or None)
    ...
    def download_from_s3(s3_uri: str, local_path: str) -> None:
        ...
        extra_args = AIRHub._default_bucket_expected_owner_args(bucket)
        AIRHub._s3_client.download_file(bucket, key, local_path, ExtraArgs=extra_args or None)
