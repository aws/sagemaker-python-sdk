...
class _ModelResolver:
    ...
    def resolve_model_info(
        ...
        if isinstance(base_model, str):
            ...
            # Check if it's a model package ARN or JumpStart model ID
                ...
                return self._resolve_jumpstart_model(
                    base_model, hub_name or get_sagemaker_hub_name()
                )
        # Handle AgentRFTJob type
            ...
            if arn and not isinstance(arn, Unassigned):
                ...
                raise ValueError(
                    "AgentRFTJob must have completed training to be used for evaluation"
                )
        # Handle BaseTrainer type
            ...
            if hasattr(base_model, "_latest_training_job") and hasattr(
                base_model._latest_training_job, "output_model_package_arn"
            ):
                ...
                if not isinstance(arn, Unassigned):
                    ...
                    raise ValueError(
                        "BaseTrainer must have completed training job to be used for evaluation"
                    )
            else:
                raise ValueError(
                    "BaseTrainer must have completed training job to be used for evaluation"
            ...
            if hasattr(base_model, "model_package_arn") or hasattr(
                base_model, "inference_specification"
            ):
    ...
    def _resolve_model_package_object(self, model_package: "ModelPackage") -> _ModelInfo:
        ...
        if (
            not hasattr(model_package, "inference_specification")
