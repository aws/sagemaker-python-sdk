...
def _find_evaluation_recipe(
    recipe_collection: list, recipe_type: str = "Evaluation", evaluation_type: Optional[str] = None
...
def _get_evaluation_override_params(
    ...
    logger.info(
        f"Searching for evaluation recipe with Type='Evaluation' and EvaluationType='{evaluation_type}'"
    )
    evaluation_recipe = _find_evaluation_recipe(
        recipe_collection, recipe_type="Evaluation", evaluation_type=evaluation_type
    )
...
def get_resolved_recipe_from_context(
    ...
    if frt is None:
        frt_candidate = (
            getattr(hyperparameters, "_full_recipe_template", None) if hyperparameters else None
        )
...
def resolve_recipe(
    ...
    """
    if not recipe_path and not overrides:
        raise ValueError("resolve_recipe() requires a 'recipe' or 'overrides' to be provided.")
...
def _list_hub_models_by_recipe(
    ...
    if recipe_type not in ("FineTuning", "Evaluation"):
        raise ValueError(f"recipe_type must be 'FineTuning' or 'Evaluation', got: {recipe_type!r}")

    keyword_base = _build_recipe_keyword(recipe_type, technique)

    region = (
        getattr(session, "region_name", None)
        or getattr(getattr(session, "boto_session", None), "region_name", None)
