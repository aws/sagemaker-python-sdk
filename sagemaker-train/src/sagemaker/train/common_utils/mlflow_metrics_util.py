...
class _MLflowMetricsUtil:
        ...
        try:
            ...
            if not self.experiment:
                raise _MLflowMetricsError(
                    _ErrorConstants.EXPERIMENT_NOT_FOUND.format(self.experiment_name)
                )
    ...
    def _list_runs(self, run_name: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        try:
            runs = mlflow.search_runs(
                ...
                filter_string=(
                    f"tags.{_MLflowConstants.MLFLOW_RUN_NAME_TAG} = '{run_name}'"
                    if run_name
                    else None
                ),
            )
    ...
    def _get_loss_metrics(
        self, run_id: Optional[str] = None, run_name: Optional[str] = None
        ...
        try:
            ...
            for rid in run_ids:
                ...
                for metric_key in run.data.metrics:
                    if any(
                        kw in metric_key.lower() for kw in _MLflowConstants.LOSS_METRIC_KEYWORDS
                    ):
    ...
    def get_metric_history(self, run_id: str, metric_name: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            return [
                {"step": m.step, "value": m.value, "timestamp": m.timestamp} for m in metric_history
            ]
        except Exception as e:
            raise _MLflowMetricsError(
                ...
                for metric in metrics:
                    ...
                    if _MLflowConstants.EPOCH_KEYWORD in metric["metric_name"].lower():
                        ...
                        for point in metric["history"]:
                            epoch = point[
                                "step"
                            ]  # Assuming step represents epoch for epoch metrics
    ...
    def _get_most_recent_total_loss(
        ...
        try:
            ...
            for rid, metrics in loss_metrics.items():
                for metric in metrics:
                    if any(
                        kw in metric["metric_name"].lower()
                        for kw in _MLflowConstants.LOSS_METRIC_KEYWORDS
                    ):
                        if metric["history"]:
    ...
    def _get_run_ids(self, run_id: Optional[str], run_name: Optional[str]) -> list[str]:
        ...
        raise _MLflowMetricsError(
            _ErrorConstants.NO_RUNS_FOUND.format(
                self.experiment_name, f" with run_name '{run_name}'" if run_name else ""
            )
        )
