...
"""Integration tests for CPTTrainer (non-data-mixing) on HyperPod (end-to-end, no mocks).
...
The test submits a real CPT job to HyperPod without DataMixingConfig and verifies
...
def test_cpt_trainer_nova_micro_hyperpod(
    ...
    The SDK auto-resolves the recipe and training image from Hub metadata, then
    submits the job via the HyperPod CLI.
    """
    ...
    cpt_trainer = CPTTrainer(
        ...
        base_job_name=f"nilis-hp-cpt-m1-{unique_id}",
    )
