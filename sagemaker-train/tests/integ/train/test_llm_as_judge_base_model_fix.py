...
class TestLLMAsJudgeBaseModelFix:
    ...
    def test_base_model_evaluation_uses_correct_weights(self, mlflow_resource_arn):
        ...
        while elapsed < max_wait_seconds:
            execution.refresh()
            step_names = (
                [step.name for step in execution.status.step_details]
            ...
            has_base_step = any(
                "base" in name.lower() and "inference" in name.lower() for name in step_names
            )
            has_custom_step = any(
                "custom" in name.lower() and "inference" in name.lower() for name in step_names
            )
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "  Results are available but display utility has a bug with None scores"
                )
    ...
    def test_base_model_false_still_works(self, mlflow_resource_arn):
        ...
        assert (
            not has_base_step
        ), f"Pipeline should NOT have base inference step when evaluate_base_model=False. Found steps: {step_names}"
        assert (
            has_custom_step
        ), f"Pipeline should have custom inference step. Found steps: {step_names}"
        ...
        logger.info(
            f"  Custom model step: {'Found (correct)' if has_custom_step else 'Missing (ERROR!)'}"
        )
