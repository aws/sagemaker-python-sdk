...
@pytest.mark.parametrize(
    ...
    [
        (
            _Benchmark.MMLU,
            ["modality", "description", "metrics", "strategy", "subtask_available", "subtasks"],
        ),
        (
            _Benchmark.BBH,
            ["modality", "description", "metrics", "strategy", "subtask_available", "subtasks"],
...
@patch("sagemaker.train.common_utils.model_resolution._resolve_base_model")
@patch("sagemaker.core.resources.Artifact")
def test_benchmark_evaluator_evaluate_invalid_subtask_override(
    mock_artifact, mock_resolve, mock_resolve_mlflow
):
...
@patch("sagemaker.train.common_utils.model_resolution._resolve_base_model")
@patch("sagemaker.core.resources.Artifact")
def test_benchmark_evaluator_hyperparameters_property(
    mock_artifact,
    mock_resolve,
    mock_get_params,
...
@patch("sagemaker.train.common_utils.model_resolution._resolve_base_model")
@patch("sagemaker.core.resources.Artifact")
def test_benchmark_evaluator_get_benchmark_template_additions(
    mock_artifact,
    mock_resolve,
    mock_get_params,
    mock_extract_options,
...
def test_benchmark_evaluator_llm_judge_nova_validation(mock_artifact, mock_resolve, mock_is_nova):
    ...
    mock_info.base_model_arn = (
        "arn:aws:sagemaker:us-west-2:aws:hub-content/SageMakerPublicHub/Model/nova-pro/1.0.0"
    )
    mock_info.source_model_package_arn = None
    mock_resolve.return_value = mock_info
...
@patch("sagemaker.train.common_utils.model_resolution._resolve_base_model")
@patch("sagemaker.core.resources.Artifact")
def test_benchmark_evaluator_template_additions_with_list_subtasks(
    mock_artifact,
    mock_resolve,
    mock_get_params,
    mock_extract_options,
...
def test_benchmark_evaluator_with_networking(mock_resolve):
    ...
    vpc_config = VpcConfig(security_group_ids=["sg-123"], subnets=["subnet-123"])
...
@patch("sagemaker.train.common_utils.model_resolution._resolve_base_model")
@patch("sagemaker.core.resources.Artifact")
def test_benchmark_evaluator_uses_metric_key_for_nova(
    mock_artifact,
    mock_resolve,
    mock_get_params,
    mock_extract_options,
...
@patch("sagemaker.train.common_utils.model_resolution._resolve_base_model")
@patch("sagemaker.core.resources.Artifact")
def test_benchmark_evaluator_uses_evaluation_metric_key_for_non_nova(
    mock_artifact,
    mock_resolve,
    mock_get_params,
    mock_extract_options,
