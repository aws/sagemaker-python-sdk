...
def _patch_supported_models(model_ids=None, side_effect=None):
    ...
    return patch(_S3_READ_FILE_PATH, return_value=_supported_models_doc(model_ids or []))
...
@patch("sagemaker.train.common_utils.model_resolution._resolve_base_model")
@patch("sagemaker.core.resources.Artifact")
def test_llm_as_judge_evaluator_get_llmaj_template_additions(
    mock_artifact, mock_resolve, mock_s3_upload
):
...
@patch("sagemaker.train.common_utils.model_resolution._resolve_base_model")
@patch("sagemaker.core.resources.Artifact")
def test_llm_as_judge_evaluator_get_llmaj_template_additions_no_metrics(
    mock_artifact, mock_resolve
):
...
@patch("sagemaker.train.common_utils.model_resolution._resolve_base_model")
@patch("sagemaker.core.resources.Artifact")
def test_llm_as_judge_evaluator_custom_metrics_only_no_builtin(
    mock_artifact, mock_resolve, mock_s3_upload
):
...
@patch("sagemaker.train.common_utils.model_resolution._resolve_base_model")
@patch("sagemaker.core.resources.Artifact")
def test_llm_as_judge_evaluator_evaluate_method(
    mock_artifact, mock_resolve, mock_resolve_mlflow, mock_execution_class, mock_pipeline
):
...
@patch("sagemaker.train.common_utils.model_resolution._resolve_base_model")
@patch("sagemaker.core.resources.Artifact")
def test_llm_as_judge_evaluator_evaluate_with_model_package(
    mock_artifact, mock_resolve, mock_resolve_mlflow, mock_execution_class, mock_pipeline
):
...
def test_llm_as_judge_evaluator_get_all(mock_execution_class):
    ...
    mock_execution_class.get_all.assert_called_once_with(
        eval_type=EvalType.LLM_AS_JUDGE, session=mock_session, region=DEFAULT_REGION
    )
...
def test_llm_as_judge_evaluator_with_vpc_config(mock_artifact, mock_resolve):
    ...
    vpc_config = VpcConfig(security_group_ids=["sg-123456"], subnets=["subnet-123456"])
...
def test_llm_as_judge_evaluator_valid_evaluator_models(mock_artifact, mock_resolve):
    ...
    mock_session = Mock()
    mock_session.boto_region_name = (
        "us-west-2"  # Region where all models including nova-pro are available
    )
    mock_session.boto_session = Mock()
    mock_session.get_caller_identity_arn.return_value = DEFAULT_ROLE
...
def test_lifecycle_active_model_passes(mock_artifact, mock_resolve):
    ...
    bedrock_client = _configure_bedrock_get_model(mock_session, lifecycle={"status": "ACTIVE"})
...
def test_evaluate_invokes_lifecycle_check(mock_artifact, mock_resolve):
    ...
    with (
        patch.object(evaluator, "_get_resolved_model_info", return_value=None),
