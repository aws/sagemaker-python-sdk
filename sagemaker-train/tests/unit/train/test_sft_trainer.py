...
class TestSFTTrainer:
    ...
    @patch("sagemaker.train.sft_trainer._validate_and_resolve_model_package_group")
    @patch("sagemaker.train.sft_trainer._get_fine_tuning_options_and_model_arn")
    def test_init_applies_constructor_hyperparameters(
        self, mock_finetuning_options, mock_validate_group, mock_session
    ):
    ...
    @patch("sagemaker.train.sft_trainer._validate_and_resolve_model_package_group")
    @patch("sagemaker.train.sft_trainer._get_fine_tuning_options_and_model_arn")
    def test_init_ignores_non_overridable_constructor_hyperparameter(
        self, mock_finetuning_options, mock_validate_group, mock_session
    ):
    ...
    @patch("sagemaker.train.sft_trainer._validate_and_resolve_model_package_group")
    @patch("sagemaker.train.sft_trainer._get_fine_tuning_options_and_model_arn")
    def test_init_with_full_training_type(
        self, mock_finetuning_options, mock_validate_group, mock_session
    ):
        mock_validate_group.return_value = "test-group"
        mock_hyperparams = Mock()
        ...
        mock_finetuning_options.return_value = (mock_hyperparams, "model-arn", False)
        trainer = SFTTrainer(
            model="test-model", training_type=TrainingType.FULL, model_package_group="test-group"
        )
    ...
    def test_peft_value_for_lora_training(
        self,
    ...
    @patch("sagemaker.train.sft_trainer._create_model_package_config")
    @patch("sagemaker.core.resources.TrainingJob.create")
    def test_peft_value_for_full_training(
        self,
        mock_training_job_create,
        mock_model_package_config,
        mock_mlflow_config,
    ...
    def test_training_type_string_value(
        ...
        mock_finetuning_options.return_value = (mock_hyperparams, "model-arn", False)
        trainer = SFTTrainer(
            model="test-model", training_type="CUSTOM", model_package_group="test-group"
        )
    ...
    @patch("sagemaker.train.sft_trainer._validate_and_resolve_model_package_group")
    @patch("sagemaker.train.sft_trainer._get_fine_tuning_options_and_model_arn")
    def test_model_package_input(
        self, mock_finetuning_options, mock_validate_group, mock_resolve_model
    ):
        mock_validate_group.return_value = "test-group"
        mock_hyperparams = Mock()
        mock_hyperparams.to_dict.return_value = {}
    ...
    def test_init_with_mlflow_config(
        ...
        assert (
            trainer.mlflow_resource_arn
            == "arn:aws:mlflow:us-east-1:123456789012:tracking-server/test"
        )
    ...
    @patch("sagemaker.train.sft_trainer._validate_and_resolve_model_package_group")
    @patch("sagemaker.train.sft_trainer._get_fine_tuning_options_and_model_arn")
    def test_fit_without_datasets_raises_error(
        self, mock_finetuning_options, mock_validate_group, mock_get_session
    ):
        mock_validate_group.return_value = "test-group"
        mock_hyperparams = Mock()
        mock_hyperparams.to_dict.return_value = {}
    ...
    @patch("sagemaker.train.sft_trainer._validate_and_resolve_model_package_group")
    @patch("sagemaker.train.sft_trainer._get_fine_tuning_options_and_model_arn")
    def test_s3_output_path_configuration(
        self, mock_finetuning_options, mock_validate_group, mock_session
    ):
        mock_validate_group.return_value = "test-group"
        mock_hyperparams = Mock()
    ...
    @patch("sagemaker.train.sft_trainer._validate_and_resolve_model_package_group")
    @patch("sagemaker.train.sft_trainer._get_fine_tuning_options_and_model_arn")
    def test_gated_model_eula_validation(
        self, mock_finetuning_options, mock_validate_group, mock_session
    ):
        ...
        mock_hyperparams = Mock()
        mock_hyperparams.to_dict.return_value = {}
        mock_finetuning_options.return_value = (
            mock_hyperparams,
            "model-arn",
            True,
        )  # is_gated_model=True
    ...
    def test_process_hyperparameters_handles_missing_attributes(self):
        ...
        mock_hyperparams = Mock()
        mock_hyperparams._specs = {"data_path": "test_data_path", "other_param": "should_remain"}
        mock_hyperparams.data_path = "test_data_path"
    ...
    @patch("sagemaker.train.sft_trainer._create_model_package_config")
    @patch("sagemaker.core.resources.TrainingJob.create")
    def test_train_passes_wait_timeout(
        self,
        mock_training_job_create,
        mock_model_package_config,
        mock_mlflow_config,
    ...
    @patch("sagemaker.train.sft_trainer._create_model_package_config")
    @patch("sagemaker.core.resources.TrainingJob.create")
    def test_train_without_wait_timeout_uses_default(
        self,
        mock_training_job_create,
        mock_model_package_config,
        mock_mlflow_config,
    ...
    @patch("sagemaker.train.sft_trainer._create_model_package_config")
    @patch("sagemaker.core.resources.TrainingJob.create")
    def test_train_wait_false_skips_wait(
        self,
        mock_training_job_create,
        mock_model_package_config,
        mock_mlflow_config,
    ...
    def test_init_with_sequence_length(self, mock_finetuning_options, mock_validate_group):
        ...
        mock_finetuning_options.return_value = (mock_hyperparams, "model-arn", False)
        trainer = SFTTrainer(
            model="test-model", model_package_group="test-group", sequence_length="8K"
        )
    ...
    @patch("sagemaker.train.sft_trainer._create_model_package_config")
    @patch("sagemaker.core.resources.TrainingJob.create")
    def test_train_passes_sequence_length_to_serverless_config(
        self,
        mock_training_job_create,
        mock_model_package_config,
        mock_mlflow_config,
...
class TestSFTTrainerComputeDispatch:
    ...
    def test_none_routes_to_serverless(self):
        ...
        with (
            patch.object(trainer, "_train_serverful_smtj") as mock_smtj,
...
class TestSFTTrainerDataMixingIntegration:
    ...
    @patch("sagemaker.train.sft_trainer._validate_and_resolve_model_package_group")
    @patch("sagemaker.train.sft_trainer._get_fine_tuning_options_and_model_arn")
    def test_sft_trainer_accepts_data_mixing_config(
        self, mock_finetuning_options, mock_validate_group
    ):
    ...
    @patch("sagemaker.train.sft_trainer._validate_and_resolve_model_package_group")
    @patch("sagemaker.train.sft_trainer._get_fine_tuning_options_and_model_arn")
    def test_sft_trainer_data_mixing_config_defaults_to_none(
        self, mock_finetuning_options, mock_validate_group
    ):
...
class TestSFTTrainerHyperPodDatamixOrchestration:
    ...
    def test_full_orchestration_call_order(
        ...
        mock_build.return_value = (
            "recipes/nova_sft_lora",
            "123456.dkr.ecr.us-east-1.amazonaws.com/image:latest",
        )
        ...
        # Track call order
        call_order = []
        mock_validate_model.side_effect = lambda *args, **kwargs: call_order.append(
            "validate_model"
        )
        mock_resolve.side_effect = lambda *args, **kwargs: (
            call_order.append("resolve"),
            mock_context,
        )[1]
        mock_validate_categories.side_effect = lambda *args, **kwargs: (
            call_order.append("validate_categories"),
            mock_validated_config,
        )[1]
        mock_build.side_effect = lambda *args, **kwargs: (
            call_order.append("build"),
            ("recipes/nova_sft_lora", "123456.dkr.ecr.us-east-1.amazonaws.com/image:latest"),
        )[1]
...
class TestSFTTrainerSmtjS3DataType:
    ...
    def _make_trainer(
        ...
        mock_resolve,
        mock_validate,
        model_name="nova-textgeneration-lite",
        compute=None,
    ):
        from sagemaker.train.sft_trainer import SFTTrainer
        from sagemaker.core.training.configs import Compute
    ...
    def _run_smtj_and_capture_input_config(self, model_name):
        ...
        mock_open.return_value.__enter__ = Mock(
            return_value=MagicMock(read=Mock(return_value="dummy: recipe"))
        )
        ...
        with (
            patch("sagemaker.train.defaults.TrainDefaults") as mock_defaults,
            patch(
            ...
            patch("sagemaker.train.base_trainer._get_smhp_instance_type_enum", return_value=None),
            patch("sagemaker.train.base_trainer._get_smhp_replicas_enum", return_value=None),
            patch("sagemaker.train.base_trainer.validate_data_path_exists"),
            patch(
                "sagemaker.train.model_trainer.ModelTrainer.from_recipe",
                mock_model_trainer_cls.from_recipe,
            ),
        ):
    ...
    def test_oss_model_uses_s3prefix_data_type(self):
        input_data_config = self._run_smtj_and_capture_input_config(
...
class TestSFTTrainerBaseModelName:
    ...
    @patch(
        ...
        return_value=("model_obj", "nova-textgeneration-lite-v2"),
    )
    def test_s3_model_with_base_model_name(
    ...
    def test_s3_model_without_compute_raises(
        self, mock_resolve, mock_validate_group, mock_get_options, mock_eula
    ):
    ...
    def test_normal_model_name_sets_no_model_source(
        self, mock_resolve, mock_validate_group, mock_get_options, mock_eula
    ):
    ...
    def test_disable_output_compression_stored(
        self, mock_resolve, mock_validate_group, mock_get_options, mock_eula
    ):
...
class TestSFTTrainerPipelineSession:
    ...
    def test_train_pipeline_session_produces_valid_step_arguments(
        ...
        mock_finetuning_options.return_value = (
            mock_hyperparams,
            "arn:aws:sagemaker:us-west-2:123:model/test",
            False,
        )
        mock_validate_group.return_value = "test-group"
    ...
    def test_train_without_pipeline_session_launches_job(
        ...
        trainer = SFTTrainer(
            ...
            training_dataset="s3://bucket/data",
            model_package_group="grp",
            sagemaker_session=regular_session,
        )
