...
class TestExtractMetricsFromResults:
    ...
    def test_extract_from_all_key(self):
        ...
        results_dict = {"results": {"all": {"accuracy": 0.95, "f1_score": 0.92}}}
    ...
    def test_extract_from_custom_key(self):
        ...
        results_dict = {
            "results": {"custom|gen_qa_gen_qa|0": {"accuracy": 0.88, "precision": 0.90}}
        }
...
class TestShowBenchmarkResults:
    ...
    def test_show_results_with_custom_and_base(
        ...
        s3_mock.list_objects_v2.side_effect = [
            {"Contents": [{"Key": f"{DEFAULT_PREFIX}/custom-job/output/output/results_test.json"}]},
            {"Contents": [{"Key": f"{DEFAULT_PREFIX}/base-job/output/output/results_test.json"}]},
        ]
    ...
    @patch("sagemaker.train.common_utils.show_results_utils._extract_training_job_name_from_steps")
    @patch("boto3.client")
    def test_show_results_no_job_names(
        self, mock_boto_client, mock_extract_job, mock_pipeline_execution
    ):
    ...
    @patch("sagemaker.train.common_utils.show_results_utils._extract_training_job_name_from_steps")
    @patch("boto3.client")
    def test_show_results_no_files_found(
        self, mock_boto_client, mock_extract_job, mock_pipeline_execution
    ):
...
class TestDownloadLLMAJResults:
    ...
    def test_download_results_success(self, mock_boto_client, mock_pipeline_execution):
        ...
        s3_mock.list_objects_v2.return_value = {
            "Contents": [{"Key": f"{DEFAULT_PREFIX}/bedrock-job-123/models/output_output.jsonl"}]
        }

        # Mock JSONL content
        jsonl_content = json.dumps(
            {"inputRecord": {}, "modelResponses": [], "automatedEvaluationResult": {"scores": []}}
        )
        s3_mock.get_object.return_value = {"Body": BytesIO(jsonl_content.encode("utf-8"))}
    ...
    def test_download_results_no_jsonl_file(self, mock_boto_client, mock_pipeline_execution):
        ...
        s3_mock.list_objects_v2.return_value = {
            "Contents": [{"Key": f"{DEFAULT_PREFIX}/bedrock-job-123/other_file.txt"}]
        }
...
class TestDisplaySingleLLMAJEvaluation:
    ...
    def test_display_without_explanations(self):
        ...
        result = {
            ...
            "automatedEvaluationResult": {"scores": [{"metricName": "accuracy", "result": 0.95}]},
        }
...
class TestDownloadBedrockAggregateJson:
    ...
    def test_download_aggregate_success(self, mock_boto_client, mock_pipeline_execution):
        ...
        aggregate_data = {
            ...
            "results": {
                "Faithfulness": {"score": 1.0, "total_evaluations": 10, "passed": 10, "failed": 0}
            },
        }
...
class TestDisplayAggregateMetrics:
    ...
    def test_display_with_base_model(self):
        ...
        base_aggregate = {
            "results": {
                "Faithfulness": {"score": 0.9, "total_evaluations": 10, "passed": 9, "failed": 1}
            }
        }
    ...
    def test_display_builtin_vs_custom_metrics(self):
        ...
        custom_aggregate = {
            "results": {
                ...
                "CustomMetric": {"score": 0.85, "total_evaluations": 10, "std_deviation": 0.03},
            }
        }
    ...
    def test_display_score_differences(self):
        ...
        custom_aggregate = {
            "results": {
                "Faithfulness": {"score": 0.95, "total_evaluations": 10},
                "Correctness": {"score": 0.80, "total_evaluations": 10},
            }
        }

        base_aggregate = {
            "results": {
                "Faithfulness": {"score": 0.90, "total_evaluations": 10},
                "Correctness": {"score": 0.85, "total_evaluations": 10},
            }
        }
...
class TestShowLLMAJResultsIntegration:
    ...
    def test_show_results_with_aggregate_and_win_rates(
        ...
        custom_aggregate = {"results": {"Faithfulness": {"score": 1.0, "total_evaluations": 10}}}
        base_aggregate = {"results": {"Faithfulness": {"score": 0.9, "total_evaluations": 10}}}
        ...
        mock_display_aggregate.assert_called_once_with(
            custom_aggregate, base_aggregate, mock_console
        )
    ...
    **Validates: Requirements 1.1, 1.4, 2.1, 2.4**
    ...
    def test_base_model_per_example_uses_correct_bedrock_job_name(
        ...
        assert download_calls[0] == call(mock_pipeline_execution, "custom-bedrock-job"), (
            f"Expected custom download with 'custom-bedrock-job', " f"got {download_calls[0]}"
        )
    ...
    def test_calculate_win_rates_receives_distinct_datasets(
        ...
        assert (
            custom_first_score == custom_scores[0]
...
class TestPreservationProperty:
    ...
    **Validates: Requirements 3.1, 3.2, 3.3, 3.4, 3.5, 3.6**
    ...
    def test_single_model_no_win_rates(
        ...
        custom_aggregate = {
            "results": {"Metric1": {"score": 0.9, "total_evaluations": result_count}}
        }
        ...
        mock_results = [
            {
                "inputRecord": {"prompt": "[{'role': 'user', 'content': 'Q'}]"},
                "modelResponses": [{"response": "['A']"}],
                "automatedEvaluationResult": {"scores": [{"metricName": "Metric1", "result": 0.9}]},
            }
        ] * result_count
        ...
        mock_results = [
            {
                "inputRecord": {"prompt": "[{'role': 'user', 'content': 'Q'}]"},
                "modelResponses": [{"response": "['A']"}],
                "automatedEvaluationResult": {"scores": [{"metricName": "M", "result": 0.5}]},
            }
        ] * total
        mock_download_results.return_value = mock_results
    ...
    def test_pagination_starts_at_correct_index(
        ...
        for call_idx, expected_i in enumerate([2, 3, 4]):
            ...
            assert (
                actual_call[0][0] == mock_results[expected_i]
            ), f"Call {call_idx}: expected result at index {expected_i}"
            assert (
                actual_call[0][1] == expected_i
    ...
    def test_show_explanations_passthrough(
        ...
        mock_results = [
            {
                "inputRecord": {"prompt": "[{'role': 'user', 'content': 'Q'}]"},
                "modelResponses": [{"response": "['A']"}],
                "automatedEvaluationResult": {"scores": [{"metricName": "M", "result": 0.8}]},
            }
        ] * 3
        mock_download_results.return_value = mock_results
        ...
        for c in mock_display_single.call_args_list:
            assert (
                c[1]["show_explanations"] == show_explanations
