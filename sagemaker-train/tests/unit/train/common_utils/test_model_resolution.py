...
class TestResolveModelInfo:
    ...
    @patch(
        "sagemaker.train.common_utils.model_resolution._ModelResolver._resolve_model_package_object"
    )
...
class TestResolveModelPackageObject:
    ...
    def test_resolve_package_no_inference_spec(self):
        ...
        with pytest.raises(
            ValueError, match="NotSupported.*does not have an inference_specification"
        ):
    ...
    def test_resolve_package_fallback_name(self):
        ...
        mock_package.model_package_arn = (
            "arn:aws:sagemaker:us-west-2:123:model-package/group-name/1"
        )
...
class TestResolveModelPackageArn:
    ...
    @patch("sagemaker.train.common_utils.model_resolution._ModelResolver._get_session")
    @patch(
        "sagemaker.train.common_utils.model_resolution._ModelResolver._validate_model_package_arn"
    )
    def test_resolve_arn_success(self, mock_validate, mock_get_session, mock_model_package_class):
        ...
        mock_model_package_class.get.assert_called_once_with(
            model_package_name=arn, session=mock_session.boto_session, region="us-west-2"
        )
    ...
    def test_resolve_arn_construct_hub_content_arn(
        ...
        expected_arn = (
            "arn:aws:sagemaker:us-west-2:aws:hub-content/SageMakerPublicHub/Model/base-model/1.0"
        )
    ...
    def test_resolve_arn_construct_hub_content_arn_private_hub(
        self, mock_validate, mock_get_session, mock_model_package_class, mock_hub_content_class
    ):
        ...
        expected_arn = (
            "arn:aws:sagemaker:us-west-2:123456789012:hub-content/sdktest/Model/mock-oss-test/0.0.1"
        )
        assert result.base_model_arn == expected_arn
    ...
    def test_resolve_arn_construct_hub_content_arn_private_hub_fallback_public(
        self, mock_validate, mock_get_session, mock_model_package_class, mock_hub_content_class
    ):
    ...
    def test_resolve_arn_no_inference_spec(
        self, mock_validate, mock_get_session, mock_model_package_class
    ):
    ...
    def test_resolve_arn_no_base_model(
        self, mock_validate, mock_get_session, mock_model_package_class
    ):
...
class TestBaseTrainerHandling:
    ...
    def test_base_trainer_with_valid_training_job(self):
        ...
        mock_training_job.output_model_package_arn = (
