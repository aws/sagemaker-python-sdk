    ...
    def test_validate_dataset_file_size_exceeds_limit(self):
        ...
        try:
            with patch("os.path.getsize", return_value=DATASET_MAX_FILE_SIZE_BYTES + 1):
                with pytest.raises(
                    ValueError, match="File size .* MB exceeds maximum allowed size"
                ):
                    DataSet._validate_dataset_file(temp_file)
        finally:
            os.unlink(temp_file)
...
class TestDataSet:
    ...
    def test_create_with_s3_location(
        self,
        mock_air_hub,
    ...
    @patch("sagemaker.ai_registry.dataset.DataSet._validate_dataset_format")
    @patch("sagemaker.ai_registry.dataset.AIRHub")
    def test_create_with_s3_location_preserves_full_path(
        self,
        mock_air_hub,
        mock_validate_format,
        mock_validate_file,
    ...
    def test_get(self, mock_air_hub):
        mock_air_hub.describe_hub_content.return_value = {
            ...
            "HubContentDocument": json.dumps(
                {"DatasetS3Bucket": "bucket", "DatasetS3Prefix": "prefix"}
            ),
...
class TestDataSetCreateWithContentMetadata:
    ...
    def test_create_skips_validation_when_content_metadata_provided(
        ...
        with (
            patch(
                "sagemaker.ai_registry.dataset.DataSet._validate_dataset_file"
