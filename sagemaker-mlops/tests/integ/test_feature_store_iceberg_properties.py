...
def test_update_only_iceberg_properties_skips_parent_update(
    feature_group_name, sample_dataframe, bucket, role
):
    """Test that update with only iceberg_properties skips the SageMaker UpdateFeatureGroup call."""
    try:
        feature_definitions = load_feature_definitions_from_dataframe(sample_dataframe)
...
def test_get_without_include_flag_has_no_iceberg_properties(
    feature_group_name, sample_dataframe, bucket, role
):
    """Test that get without include_iceberg_properties leaves iceberg_properties as None."""
    try:
        feature_definitions = load_feature_definitions_from_dataframe(sample_dataframe)
        ...
        retrieved = FeatureGroupManager.get(feature_group_name=feature_group_name)
        assert retrieved.iceberg_properties is None
    finally:
        cleanup_feature_group(feature_group_name)


def test_update_iceberg_properties_overwrites_previous_values(
    feature_group_name, sample_dataframe, bucket, role
):
    """Test that updating an iceberg property overwrites its previous value."""
    try:
        feature_definitions = load_feature_definitions_from_dataframe(sample_dataframe)
