...
def test_hyperparameter_tuning_e2e(sagemaker_session, role, mnist_data_dir):
    ...
    try:
        ...
        s3_data_uri = sagemaker_session.upload_data(
            path=mnist_data_dir, bucket=bucket, key_prefix=f"{prefix}/data"
        )

        # Configure source code
        source_code = SourceCode(
            source_dir=os.path.join(os.path.dirname(__file__), "code"), entry_script="mnist.py"
        )
        ...
        # Get training image
        training_image = (
            f"763104351884.dkr.ecr.{region}.amazonaws.com/pytorch-training:1.10.0-gpu-py38"
        )
        ...
        # Prepare input data
        training_data = InputData(channel_name="training", data_source=s3_data_uri)

        # Start tuning job
        tuner.tune(inputs=[training_data], wait=False)
