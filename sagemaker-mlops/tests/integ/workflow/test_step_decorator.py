...
from sagemaker.core.workflow.conditions import ConditionLessThan, ConditionEquals
...
from sagemaker.core.shapes.shapes import ProcessingInput, ProcessingS3Input
...
from tests.integ.workflow.helpers import (
...
@pytest.fixture
def pipeline_name():
    return unique_name_from_base("Decorated-Step-Pipeline")
...
def test_passing_different_pipeline_variables_to_function(
    ...
    script_path = os.path.join(DATA_DIR, "workflow", "dummy_script.py")
    ...
    step_args = sklearn_processor.run(code=script_path)
...
def test_decorator_step_data_referenced_by_other_steps(
    ...
    script_path = os.path.join(DATA_DIR, "workflow", "dummy_script.py")
    input_file_path = os.path.join(DATA_DIR, "workflow", "dummy_input.txt")
    inputs = [
        ProcessingInput(
            input_name="input-1",
            s3_input=ProcessingS3Input(
                s3_uri=input_file_path,
                local_path="/opt/ml/processing/inputs/",
    ...
    try:
        ...
        execution_proc_job = pipeline_session.sagemaker_client.describe_processing_job(
            ProcessingJobName=execution_steps[0]["Metadata"]["ProcessingJob"]["Arn"].split("/")[-1]
        )
        assert (
