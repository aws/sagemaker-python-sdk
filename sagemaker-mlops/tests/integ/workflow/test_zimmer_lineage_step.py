...
"""Integration test for the Zimmer LineageStep.
...
This test represents the SDK-side end-to-end validation of the entire
Zimmer step family. See ``bedrock_steps.py``, ``endpoint_step.py``, and
``inference_component_step.py`` for the other step types; those are not
integ-tested here because they either provision paid resources
(Endpoint/InferenceComponent/Bedrock ProvisionedThroughput) or need
external test data (real trained model artifacts for BedrockModelImport).
"""
...
def test_lineage_step_execute_end_to_end(sagemaker_session, pipeline_session, role):
    ...
    pipeline_name = f"zimmer-integ-lineage-{stamp}"
            ...
            "Actions": [
                {
                    ...
                    "Description": "Zimmer integ test action",
                }
            ]
        },
    )
