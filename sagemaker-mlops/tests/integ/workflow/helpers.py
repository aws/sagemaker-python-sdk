...
def wait_pipeline_execution(execution: PipelineExecution, delay: int = 30, max_attempts: int = 60):
    try:
...
def create_and_execute_pipeline(
    ...
    actual_status = execution_steps[0]["StepStatus"]
    failure_reason = execution_steps[0].get("FailureReason", "")
    assert actual_status == step_status, (
        f"Expected step status '{step_status}' but got '{actual_status}'. "
        f"Step: {execution_steps[0]['StepName']}. "
